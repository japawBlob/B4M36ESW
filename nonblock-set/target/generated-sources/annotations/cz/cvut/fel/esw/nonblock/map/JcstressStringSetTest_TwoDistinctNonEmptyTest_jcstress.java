...
import cz.cvut.fel.esw.nonblock.map.JcstressStringSetTest.TwoDistinctNonEmptyTest;
import org.openjdk.jcstress.infra.results.I_Result;

public final class JcstressStringSetTest_TwoDistinctNonEmptyTest_jcstress extends Runner<I_Result> {

    volatile WorkerSync workerSync;
    TwoDistinctNonEmptyTest[] gs;
    I_Result[] gr;

    public JcstressStringSetTest_TwoDistinctNonEmptyTest_jcstress(ForkedTestConfig config) {
        super(config);
    }
    ...
    private void sanityCheck_API(Counter<I_Result> counter) throws Throwable {
        final TwoDistinctNonEmptyTest s = new TwoDistinctNonEmptyTest();
    ...
    private void sanityCheck_Footprints(Counter<I_Result> counter) throws Throwable {
        config.adjustStrideCount(new FootprintEstimator() {
          public void runWith(int size, long[] cnts) {
            ...
            TwoDistinctNonEmptyTest[] ls = new TwoDistinctNonEmptyTest[size];
            I_Result[] lr = new I_Result[size];
            for (int c = 0; c < size; c++) {
                TwoDistinctNonEmptyTest s = new TwoDistinctNonEmptyTest();
    ...
    public ArrayList<CounterThread<I_Result>> internalRun() {
        ...
        gs = new TwoDistinctNonEmptyTest[len];
        gr = new I_Result[len];
        for (int c = 0; c < len; c++) {
            gs[c] = new TwoDistinctNonEmptyTest();
            gr[c] = new I_Result();
        }
    ...
    private void jcstress_consume(Counter<I_Result> cnt, int a) {
        ...
        for (int c = left; c < right; c++) {
            I_Result r = lr[c];
            TwoDistinctNonEmptyTest s = ls[c];
            s.size(r);
            ls[c] = new TwoDistinctNonEmptyTest();
            cnt.record(r);
            r.r1 = 0;
        }
    }
    ...
    private void run_actor1(TwoDistinctNonEmptyTest[] gs, I_Result[] gr, int start, int end) {
        TwoDistinctNonEmptyTest[] ls = gs;
        I_Result[] lr = gr;
    ...
    private void run_actor2(TwoDistinctNonEmptyTest[] gs, I_Result[] gr, int start, int end) {
        TwoDistinctNonEmptyTest[] ls = gs;
        I_Result[] lr = gr;
        for (int c = start; c < end; c++) {
            TwoDistinctNonEmptyTest s = ls[c];
