...
import cz.cvut.fel.esw.nonblock.map.JcstressStringSetTest.TwoEqualTest;
import org.openjdk.jcstress.infra.results.I_Result;

public final class JcstressStringSetTest_TwoEqualTest_jcstress extends Runner<I_Result> {

    volatile WorkerSync workerSync;
    TwoEqualTest[] gs;
    I_Result[] gr;

    public JcstressStringSetTest_TwoEqualTest_jcstress(ForkedTestConfig config) {
        super(config);
    }
    ...
    private void sanityCheck_API(Counter<I_Result> counter) throws Throwable {
        final TwoEqualTest s = new TwoEqualTest();
    ...
    private void sanityCheck_Footprints(Counter<I_Result> counter) throws Throwable {
        config.adjustStrideCount(new FootprintEstimator() {
          public void runWith(int size, long[] cnts) {
            ...
            TwoEqualTest[] ls = new TwoEqualTest[size];
            I_Result[] lr = new I_Result[size];
            for (int c = 0; c < size; c++) {
                TwoEqualTest s = new TwoEqualTest();
    ...
    public ArrayList<CounterThread<I_Result>> internalRun() {
        ...
        gs = new TwoEqualTest[len];
        gr = new I_Result[len];
        for (int c = 0; c < len; c++) {
            gs[c] = new TwoEqualTest();
            gr[c] = new I_Result();
        }
    ...
    private void run_actor1(TwoEqualTest[] gs, I_Result[] gr, int start, int end) {
        TwoEqualTest[] ls = gs;
        I_Result[] lr = gr;
    ...
    private void run_actor2(TwoEqualTest[] gs, I_Result[] gr, int start, int end) {
        TwoEqualTest[] ls = gs;
        I_Result[] lr = gr;
        for (int c = start; c < end; c++) {
            TwoEqualTest s = ls[c];
